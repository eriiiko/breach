...
int64_t temperature_step(
    ...
    // P-E2a (design §2.3): out-param for the SIX energy counters, accumulated
    // (+=) into the caller's TemperatureSolver fields so telemetry is identical
    // whichever backend ran. Slot order is PINNED and mirrored by the C_* enum
    // in cuda_temperature.cu and by the CPU field order:
...
constexpr int TEMPERATURE_ENERGY_SLOTS = 6;
