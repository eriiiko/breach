...
void Raycaster::cast_source_directional(
    ...
    // T6 (issue #12): this used to also run a SECOND, SEPARATE march per ray
    // (march_ray_radiation, at `radiation_range`) when a non-null `rad`/`rs`
    // was supplied — the P-R4/P-F1a "light plus the exchange" contract. Both
    // are deleted; `rad`/`rs` stay in the signature (this IS the kept render
    // march, raycaster.{h,cpp} staying until P6) but were already the ONLY
    // way to reach that branch, and neither caller of this function ever
    // passed a non-null one (the pybind binding has no rad/rs argument at
    // all, relying on the defaults below; the other caller was
    // cast_from_fire_plane's own light-only sub-cast, which always passed
    // nullptr for both) — so the branch was unreachable dead code even
    // before this patch, not merely dead after it.
    (void)rad; (void)rs;
}
