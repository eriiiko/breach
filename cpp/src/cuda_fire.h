...
// (fire_simulation.cpp ~44-267): the per-tile signed-logistic intensity FEEDBACK
// (ignition/spread is elsewhere) → own-tile plume overpressure → smoke emission
// into neighbours → wall burn-through → final clamp. Five passes, launched as a
...
//   P2 logistic spread    fire += dt*(grow-die); snap-extinguish below I_min
//   P3 plume pressure     atmosphere[i] += max(gain, 0)            (own-tile)
...
//
// The synced fields fire, atmosphere, smoke, wall_hp (int32 Q16.16) come out
...
//
// temperature / wind_x / wind_y / is_wall / is_vacuum / flammable are READ-ONLY.
//
...
std::vector<std::pair<int, int>> fire_step(
    ...
    int32_t* atmosphere,       // Q16.16 (h,w) — in/out (plume deposit, own-tile)
    int32_t* smoke,            // Q16.16 (h,w) — in/out (emission scatter into nbrs)
    int32_t* wall_hp,          // Q16.16 (h,w) — in/out (burn-through depletion)
    const int32_t* temperature,// Q16.16 (h,w) — read-only (the `hot` gate)
    ...
    // FireParams dials (verbatim the CPU `params`):
