...
SCENARIO (isolates fire from O2 starvation, per Erik):
  ...
  * ONE furniture crate (material `furniture`: hp 30, ignition_temp 280,
    flammable), placed ADJACENT to the ambient ring (default crate x=1). This is
    LOAD-BEARING (a bench finding): a crate deeper in the room O2-STARVES -- the
    burning tile thermally thins its local O2 density faster than the reservoir
    can replenish it -- and stalls at the re-ignition smolder (I pinned 0.1). A
    ring-adjacent crate keeps its flame-neighbour O2 density ~0.05, ABOVE
    P_full=0.03, so the fire's O2 gate SATURATES (O2-unlimited) and it ramps and
    burns on fuel + thermal + k_grow/k_die alone -- the isolation we want.
...
def build_level(interior_w, interior_h, crate_xy, tile_size_m):
    ...
    air is directly bounded by the ambient sky (open field -> O2 stays oxygenated
    except right at the flame)."""
...
def _run_one_inner(wind_dq, *, interior_w, interior_h, crate_xy, tile_size_m,
    ...
    far_xy = (cy, min(w - 2, interior_w // 2 + 1))
    ...
    for k in range(1, n_max + 1):
        ...
        o2far = int(gmap.gas[O2, far_xy[0], far_xy[1]]) / FP_ONE
...
def write_timeseries_csv(m, path):
    ...
    with open(path, "w", newline="") as f:
        ...
        w.writerow(["t_s", "t_min", "I", "T_game", "wall_hp",
                    "O2_flame_nbr", "O2_gate", "O2_far", "soot_deq"])
...
def _print_run(m):
    ...
    print(f"  scenario: interior {m['interior_w']}x{m['interior_h']}, crate at "
          f"{m['crate_xy']} (ring-adjacent), tile {m['tile_size_m']} m; PLANETSIDE O2=0.21")
    print("-" * 76)
    if m["stalled"]:
    ...
    print(f"  O2 gate over burn:          {sat}  (flame-nbr O2 min {m['o2_min']:.4f}, "
          f"far/reservoir O2 min {m['o2far_min']:.4f})")
    if m["forced"]:
...
def main(argv=None):
    ...
    ap.add_argument("--interior-w", type=int, default=60)
    ap.add_argument("--interior-h", type=int, default=20)
    ap.add_argument("--crate-x", type=int, default=1,
                    help="crate column; default 1 = adjacent to the upwind ambient ring")
