"""Level format v2 [art] block + align transform (F2).
...
level_loader.py:
  ...
  - [art.align]: offset_px + px_per_tile parse; px_per_tile defaults to
    art_width / grid_width (read from the PNG header) when absent; malformed
    values raise.
...
def test_freeze_unhcr_vessel_flat_keys():
    ...
    # Default px_per_tile = art_width / grid_width = 1000 px / 50 tiles —
    # exactly the implicit transform of the legacy stretch-to-grid draw
    # (diffuse.png is 1000x2400 over a 50x120 grid: 20 px/tile both axes).
    assert lvl.art_px_per_tile == pytest.approx(20.0)
...
def test_freeze_version_1_level(tmp_path):
    ...
    assert lvl.art_align_explicit is False
    assert lvl.art_offset_px == (0.0, 0.0)
    assert lvl.art_px_per_tile == pytest.approx(40 / 4)   # art_w / grid_w
...
def test_v2_art_block_parses(tmp_path):
    ...
    assert lvl.art_px_per_tile == pytest.approx(12.5)
...
def test_align_defaults_and_validation(tmp_path):
    ...
    assert lvl.art_align_explicit is False
    assert lvl.art_offset_px == (0.0, 0.0)
    assert lvl.art_px_per_tile == pytest.approx(64 / 4)
    ...
    assert lvl.art_px_per_tile == pytest.approx(16.0)
...
def test_unhcr_vessel_2_loads_end_to_end():
    ...
    assert lvl.art_align_explicit is True
    assert lvl.art_offset_px == (0.0, 0.0)
    assert lvl.art_px_per_tile == pytest.approx(78.0)   # 3900 px / 50 tiles
