...
Folded into `docs/swarm_P3_impl.md` v2 (2026-09-24) by the synthesis step,
under the orchestrator's decisions 1–10 and three rulings Erik gave during the
synthesis (E1 = RAW; the larva is much smaller than a tile; build paused until
fire-12 merges). Every resolution is applied in the body of v2, not appended.
Re-probes run for the synthesis (CPU build, HEAD `ea4c58f`) are v2 Appendix
A3′ and A4.

| R# | findings | resolution | doc § |
|---|---|---|---|
| R1 | L2-M1 (+ decision 2), L1-m5 | v1's V1 (runner-only `_run_swarm`) WITHDRAWN. Orchestration moves into a new `PhysicsEngine::step_swarm` (body `swarm::step_all` in the strict, 0/0/0 swarm TU — the `run_substeps_resident` forwarding idiom): species walk, presence over EVERY env, field and dirty-store uploads, launch, store D2H, (species, env, slot)-ordered death compaction. The runner calls it in ONE statement after `step_tail` on both tick paths; Python keeps only the ingress gather `swarm.engine_args` and the rows → `SwarmUnitDiedEvent` conversion; the backend choice is read in C++ (the `step_tail` precedent). §9 merge table redone: +1 include and +1 declaration block in `physics_engine.h` (quiet span), one forwarding definition in `physics_engine.cpp` (quiet span 386-936), one `.def` in `bindings.cpp` (quiet span); no float/double token added. | V1, D14, D17, §3.6, §4, §9, §10, §16 |
| R2 | L1-M1 | `resident` is passed explicitly (`True` only from `_step_resident`); the resident branch uploads `temperature` AND `solid` itself (self-contained); the host `high_water[e]` rides in each env's launch block and is the one source for the kernel mask, the launch extent and the compaction (the device `high_water` copy is never read). PART 2b scripts a mid-run `destroy_wall` next to a larva. | V6, D16, §4.2, §4.4, §11 PART 2b |
| R3 | L1-B1 | PART 1's inputs satisfy the kernel contract (stated in §3.5): per env `check_invariants` holds on `[0, hw[e])`, headings from `(−PI_Q16, PI_Q16]` with extremes `−PI_Q16+1`/`PI_Q16`, in-grid positions, `unit_id < next_unit_id ≤ INT32_MAX`; a dedicated garbage-beyond-hw forcer is identity-only. Both CUDA wrappers `return 0` on `n_env == 0 || launch_hw == 0` before any allocation/transfer/launch. | §3.5, §3.6, §11 PART 1 |
| R4 | L1-M3 | PART 1 fuzzes `n_env ∈ {1, 2, 3}` with unequal `hw[e]` incl. 0; both CUDA wrappers D2H records `[0, hw[e])` per env only, and the death count/compaction scan the same range; columns and records are pre-filled with identical garbage and compared in full. | §3.6, §4.4, §11 PART 1 |
| R5 | L1-M2 (+ decision 4) | `match_key`: `operator.index(entropy)` (numpy ints accepted, list/float → `TypeError`); a non-empty `spawn_key` → loud `ValueError` — ACCEPTED GAP, the RL arc folds `spawn_key` in when it needs sibling seeds. T-P3-33 covers `np.int64`, a spawned child, a list. | D13, §0, §6, §11 T-P3-33 |
| R6 | L2-M3, L1-m4 (+ decision 5) | One `Simulation.total_tick` property (`round_index * ticks_per_round + round_tick`) is the Philox clock; the conductor passes `PhiloxClock(*self._philox_key, self.total_tick)` in one line; T-P3-27 asserts it under TwoPhase and OnePhase. `level_lights.monotonic_total_tick` untouched (recorded). The save contract must carry what `total_tick` depends on (routed to ch.17 §6). Draft CLAUDE.md row "Absolute tick". | V5, D18, §6, §9 routing, §16 |
| R7 | L2-B1 (+ decision 3), F8 | No `SWARM_COUPLING_TABLE`. `CouplingRow` gains `response: Optional = None`, `consumer: str = "unit"`, `reads: tuple = ()`; three `consumer="swarm:larva"` rows (thermotaxis, heat kill, coma) are appended to `COUPLING_TABLE`; the future executor iterates `consumer == "unit"` only. The three snapshot tests in `test_exchange_reductions.py:260-300` are rewritten as properties (listed as pre-existing-test changes, each with its property and breaking change). T-P3-22 tightened: `reads` (as `<name>_q`) ⊆ the kernel's params and `(field, reduction)` equals the law registry's. F8 corrected (`test_wave_push` survives an append). The "Swarm coupling rows" draft rule is dropped; the "Coupling table" row is amended. | F8, §8, §11, §16 |
| R8 | L2-B2 | T-P3-15 is PAIRED (law vs ablated, identical placements/headings/uids/seeds/ticks); T-P3-17 compares the ablated law on the gradient vs on a flat field, paired; T-P3-16 places larvae symmetrically about the centre; the tumble-frequency measurements (T-P3-9, T-P3-16) are single-tick independent trials (many uids, one configuration, arrays restored per trial, probe unblocked). | §11 T-P3-9, 15-17 |
| R9 | L2-M2 (+ decision 8) | Species-agnostic machinery in shared files: `swarm.h` (kit, types, host loop template, registry type), `swarm.cpp` (registry, `step_all`, compaction, CPU counter), `swarm_launch.cuh` (kernel/per-call/resident templates), `swarm.cu` (backend flag, counters, scratch, transfers). Larva-only: `swarm_larva.{h,cpp,cu}` (the law, its POD, its draw table) and `swarm_larva.py` (param/derived order, tumble derive, rules R2-R4). `SPECIES_RULES` is a dict keyed by species (R1 shared), with an import-time `set(SPECIES_RULES) == set(SWARM_SPECIES)`; the known-limit note names the still-shared `SPECIES_FIELDS`. | §0, §3.6, §5.3, §5.4, §16 |
| R10 | L2-M4, F7 | T22's rejection cases match the offending key (structural cases their own token); the testkit's `swarm_cfg()` carries `physics.temperature_scale` from CFG; one positive case; the four P2 helpers keep `max_units=64/8192`, `hp_max=1.0` as overrides. | §11 pre-existing test changes |
| R11 | L1-m1, L2-M5 (+ decision 6) | `SIM_HEADERS = ("swarm.h", "swarm_larva.h", "philox_streams.h", "swarm_launch.cuh")` held at 0/0/0 by a new hard test; `swarm.cpp` + `swarm_larva.cpp` in `SIM_TUS` and `MIGRATED_FLOOR_TUS` at 0/0/0. `bindings.cpp` (`/fp:fast`) reaches the law only through out-of-line entries in strict TUs (fire-12's `emissive_table.cpp` precedent). The repo-wide "no header is scanned" gap is recorded, not fixed. | F6, §9, §10, §16 |
| R12 | L2-M6 (+ decision 9) | Re-probed: v1's A3 "bulk ≈ 7 °C" does not reproduce (21.0 °C, same geometry — A3′). §12 recalibrated on probe A4 to lawful `wave_source` holds: a 40 °C warm strip along the west wall + a lethal 70 °C corner + a 2×2 cold patch clamped at `T_ctmin − 5 K`. The tool asserts (1) the bulk warm-up mean lies in `[T_coma_exit + 3 K, T_prefer]` (A4: 22.6 °C, passes) and (2) < 20 % of bulk cells read below `T_ctmin` (A4 on HEAD: 31.4 %, FAILS — the recorded gas-T pattern), exits non-zero otherwise, and cross-checks its numpy RAW felt-T against every kernel coma/wake/kill decision. If (2) still fails on the merged code it goes to Erik — never tuned around. | F11, §12, App. A3′/A4 |
| R13 | L2-m1, L1-m7 (FOOTPRINT half); Erik's E1 ruling | E1 = RAW (Erik 2026-09-24). The FOOTPRINT reader, the E1 box, the `"mean"` label and every "works under either reader" requirement are deleted; L2-m1 and the FOOTPRINT half of L1-m7 are moot. The reader is a field-generic `TileReader`, labelled `"center"` (true: `reduce_center` over one tile), exported by the law registry and tied to the coupling rows by T-P3-22; T-P3-31 pins it. | Ruling 11, §3.2, §8, §11 |
| R14 | L2-m2 | Launch words (`step_q`, `p_base_q`, `p_alarm_q`) are derived on the host at every launch, and the species rules re-run on exactly those ints; the kernel never derives. Removed: the C++ derive, the `swarm_larva_derived` binding, T-P3-20, `ENV_DT_Q`/`ENV_INV_TILE_Q`, the Python `LARVA_SWEEP_DIST_Q16`. | D15, §3.4, §4.3, §5.3 |
| R15 | L1-m2 | No `std::` helper in any FP_HD function; hand-written ternaries (`abs64`). | §3 preamble, §3.1 |
| R16 | L1-m3 | `check_invariants` gains the rule "live `pos` ∈ `[0, w<<16) × [0, h<<16)`" (pure reader, no digest effect); T-P3-32 proves it non-vacuous; the kernel contract states the precondition. | §3.5, §11 T-P3-32, §13 step 9 |
| R17 | L1-m6 | The Kelvin conversion re-raises `RuntimeError` AND `AssertionError` from `temperature_scale.load` as the seam's `ValueError`; T-P3-24 covers the assertion branch. | §5.2, §11 T-P3-24 |
| R18 | L1-m7 (staircase half), L2-m6 | T-P3-1's staircase is 4-connected, plus a positive corner-cut case; T-P3-3 calls the real `check_invariants` on a duck-typed store (`testkit.as_store_namespace`); PART 2's synthetic `T` and walls are standalone arrays, never `gmap.temperature`/`gmap.solid`; T-P3-7's geometry is specified for the RAW reader; T-P3-23 uses `swarm_fixed.dequantize`. | §11 |
| R19 | L1-m8, L2-m7 | PART 2b is three-way (CPU / per-call through the sim / resident); new counters `swarm_store_uploads` (asserted to tick exactly on the first resident tick and after each scripted facade write) and `swarm_field_uploads`; a scripted mid-run `reclaim` of a live larva must stay reclaimed on every world; a swarm-free resident leg asserts no resident call and no upload. | §4.5, §10, §11 PART 2b |
| R20 | L2-m3 | `swarm_thermal_scenario_sim` is justified (P2's T8 requires `swarm_scenario_sim`'s fields to equal the default scenario's, and it never drives coma); P2's T7/T8/T9 are listed as must-stay-green (now kernel-active). The xarch run reuses `capture_trajectory` + `build_perfield_lines` + `first_divergence`, adds only `build_swarm_lines`, and interleaves per tick so a field divergence is reported first; the deaths line is dropped (a death event is a pure function of the previous tick's digested section). | §11 thermal scenario + xarch |
| R21 | L2-m4 | Each deviation is routed into the passage it amends: V4 → ch.17 §4.1 step 3; V5 → ch.17 §3, the ch.14 door-4 amendment, the CLAUDE.md "Deterministic RNG" row; the swarm step → ch.17 §5 "As built"; `total_tick` → ch.17 §6; F5 → RL proposal §B2. The draw table lives in `swarm_larva.h`'s header comment; the rules point there. | §3.5, §9 routing, §16 |
| R22 | L2-m5 | The death-event rule now says: renderers read the event; a sim-side consumer (P6 food) acts at the emission point, and if one ever reads the event the type joins the synced event digest in that patch. | §7, §16 |
| R23 | Erik 2026-09-24: larva-size premise | Every 0.6 m / body-length derivation removed (the FOOTPRINT "body footprint", the `speed_mps` rationale). `speed_mps = 0.2` is a plain P4 placeholder satisfying `speed·Δt < 1 tile`; the sweep's "1 tile ahead" is stated as a field-grid sampling distance, not a body length; no plot glyph is size-scaled; the size is Erik's to give before P4/P5. | Ruling 12, D2, §0, §5.5, §12, §14 |
| R24 | Erik 2026-09-24: pause | Status header: design complete, build PAUSED until fire-12 merges; §13 step 0 is a refresh pass (anchors, §9, the k 3→1 Kelvin ints, re-run A1/A4, red baseline) before the patch branch is cut. | header, ruling 13, §13 |

### Findings in other systems (recorded, not P3)

- `level_lights.monotonic_total_tick` (`level_lights.py:55-65`) is ruleset-blind and double-counts under OnePhaseWEGO (render beacons jump `ticks_per_round` ticks at every round seam: `main.py:532`, `renderer/frame_lights.py:35`, `tools/lighting_demo.py:943/1612`); its callers should move to `sim.total_tick` — recorded, not P3.
- `COUPLING_TABLE` snapshot tests (`test_exchange_reductions.py:260-300`) pinned the field list and a 5-tuple unpack of a table designed to grow — fixed in P3 by R7 (rewritten as properties); recorded as the incident.
- Backend-setter lists are duplicated per script (`tools/run_on_cuda.py:92-108`, `tests/cuda_s8a_check.py:64`, `tests/cuda_sky_exchange_check.py:38`, …), and `run_on_cuda.py` omits `set_combustion_backend`, so `--cuda` never runs combustion on the GPU (v1 F9) — recorded, not P3.
- The float ratchet scans no header anywhere: every FP_HD-inline sim law in `cpp/src/*.h` (e.g. `gas_energy.h:56-75`) is outside it; P3 adds only its own headers (R11) — recorded, not P3.
- `GameMap.residency_on()` is never cleared while `physics_runner.set_residency(False)` is a public toggle; code keying on the map flag inherits L1-M1's hazard (P3 no longer does, R2) — recorded, not P3.
- `check_invariants` had no position-in-grid rule although spawn enforces one — fixed in P3 (R16); recorded as the incident.
- Grid-scale gas-T pattern after a one-shot seed in a sealed room (v2 Appendix A2; on HEAD it persists as 60-s time-means, A4); re-measure after fire-12 merges — recorded, not P3.
- `SPECIES_FIELDS` is one tuple shared by every swarm species (P2 design), so larva fields would be required of a second species; `SPECIES_RULES` is per species from P3 on, the field set is not — recorded, not P3.
- The density study (`docs/larva_density_study_2026-09-09/`) and ch.17 fork 2b's capacity reasoning ("1k = infestation, hull ceiling ~2.5k" → `max_units` 8,192) were computed for 0.6 m larvae; `max_units` is a load-time knob, so nothing in P3 changes — recorded, not P3.
- fire-12's D4 fan phase keys on the round-local `sim.tick`, so the fan phase repeats every TwoPhase round (v1 F3); it should read `sim.total_tick` once that exists — recorded, not P3.
- CLAUDE.md "Python: always the conda env `data`" is Lenovo-true, desktop-false (the cp311 `.pyd` needs the base interpreter here; v1 F10) — recorded, not P3.
