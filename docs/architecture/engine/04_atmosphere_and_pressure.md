...
- **Sealed border / hull** tiles are `is_vacuum` **and** `obstacles`/`is_wall`. Because they are
  obstacles, the Neumann reflection blocks waves and diffusion: air cannot leak through an intact
  hull. A sealed ship holds its pressure indefinitely.
