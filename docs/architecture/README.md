...
| 15 | [Level format & authoring](engine/15_level_authoring.md) | ⚠️ (format v2 + painted-path tool shipped; tiled path SHIPPED — greybox tileset + autotile baker + standalone map editor + [[light]]/beacon entities + [water] aquarium seeding, P1–P5 merged 2026-07-08; AI-styled tilesets owed; doors-v1 superseded by entity doors, ch. 16) |
| 16 | [Entity system](engine/16_entity_system.md) | ⚠️ (Arc A foundation SHIPPED 2026-07-19 — registry-in-code, [[entity]]+ids, level_lib data layer, entity digest sections, seal/unseal primitives, doors v0 (MAT_DOOR_CLOSED), zones/air_init/boundary format; SignalBus/sensors/logic = Arc B, editor UX = Arc C) |

### Mechanics — game logic on the engine
| # | Chapter | Status |
|---|---------|--------|
| 01 | [Units](mechanics/01_units.md) | ⚠️ (mobility speed_fn seam shipped; footprint variants/cadence owed) |
| 02 | [AI & line-of-sight](mechanics/02_ai_and_los.md) | ⚠️ |
| 03 | [Weapons & combat resolution](mechanics/03_combat_and_weapons.md) | ⚠️ (framework designed 2026-07-05: 6 delivery archetypes + tables + the accuracy trinity; **W1 tables + W2 + W3 shipped** — unified march, aim/snap cones, exposure/crit live, Lance-3 HITSCAN; **W3: payload executor (frag/breach replica-proven byte-identical), gas grenades (smoke/tear/poison), GL-6 40 mm detonate-at-stop, C4, mag/reload economy** — golden unchanged all three patches (W3 adds zero RNG consumers); **W4 SPRAY shipped (merged `5594650`)** — Dragon-7 flamethrower + Miasma Vent (integer cone membership, has_los occlusion, 1/dist falloff into heat/gas FieldEdits; damage rides the existing heat/poison exchange rows; `[marine] weapon` data-driven), golden unchanged (W4 draws zero RNG); **W5 MELEE shipped (merged `8f493a8`)** — combat knife + arc baton through the W2 resolver (to-hit trivially 1.0 — no exposure roll exists at touching footprints; knife crit 0.15 × facing arcs; Chebyshev-1 footprint adjacency incl. diagonals; baton → STUNNED 1.5 s at the delivery site, packets stay damage-only; zombie bite untouched on ai_zombie), golden unchanged (crit-0 melee draws nothing even while swinging); **W6 THE ARMORY built on branch `weapons-w6-armory`, awaiting Erik's grand tuning session** — METER-BASED ranges (`range_m` → tiles ONCE at level load off `tile_size_m`; the pinned 1.0 m/tile worlds keep bit-identical tile ranges, the 0.333 playground derives 3×; dragon_7 deliberately rescaled to 10 m + heat 2400), the full §6 armory as data (P12/MP-11/LR-50/Jackhammer-8/Lance-5/Sunspot/Helios + Dragon-9 heavy + incendiary rounds; `default_ammo` static round selection), plasma = direct-hit HEAT + the GL-6 detonate-at-stop splash (payload `heat_amount` heat disc + ignite ring), the SPRAY-JET + plasma-glow visuals (render-only SprayJetEvent / ProjectileGlowEvent + transient lights), the `N` weapon-cycle debug key through the facade, 626 green (+20 W6 tests), golden unchanged; beam glow-as-light + per-unit ammo-selection UI deferred) |
