...
| **R10** | **Conduction runs at REAL physical rates.** The absolute rate stops being a CFL stability anchor and becomes `α·Δt/Δx²` per material. | Erik, 2026-09-19: *"Let's stick with trying to stay close to physics… wood may be fully insulating, it's OK — we WILL have lots of other means for temp to travel. 36 h across a tile of wood, I don't mind if you set it to 0."* Resolves L1-B2. **REFINED BY T3 — R10 APPLIES TO SOLID–SOLID FACES ONLY.** A solid–gas face is governed by ledger 7a's convection coefficient, not by pure conduction, and T3 measured that **the SHIPPED rate there is already right**: face shift 10 implements `h = 6.75 W/(m²·K)` against the derived 6.0, and both round to the same shift. So this document's earlier *28–139× too weak* was true of the LAW and false of the shipped RATE — and applying R10 naively at the wall would have taken those faces from accidentally-correct to **64× too weak**, since pure conduction wants shift 17. Nothing to fix at the wall; what changes is the justification, from accident to derivation. **Consequence, accepted**: solid–solid conduction becomes a slow background process (steel 4.4e-6 per tick, wood 8.1e-8, glass 1.9e-7), and fire spread is carried by radiation and hot gas — which is what actually carries it in a real compartment fire. **Very likely closes #61**: the ~3 s e-fold draining wood's ignition heat *is* the ~43 000× overspeed. |
...
| **R14** | **Materials are authored by DENSITY; every tile-dependent number is derived.** A `[materials.*]` row states real physical properties (ρ, and the `ρc` that follows); `thermal_mass`, fuel mass and the emission area are **computed at load** from the level's own `tile_size_m` and deck height. **All base materials are MASSIVE** — a tile is a filled block, `tile_size² × ceiling_h`. Thin variants (a thin wooden wall, panelling) are authored later as their OWN rows, not as a fill fraction on a massive one. | **Erik, 2026-09-19.** Two motives, both his. (1) *"tiles matter, solid massive tiles matter, depend on the resolution as well — perhaps it would be better to define densities"*: authoring the density makes the table **resolution-independent**, so the spatial resolution never has to be locked in (*"i better fix the spatial resolution… but i wont do it before i have to"*). (2) It resolves the tension T3 found. **`thermal_mass` already says MASSIVE on every row** — steel 32 *is* solid steel's `ρc`, wood 8 *is* solid wood's — which is exactly why T3's D1 was a no-op. So this ruling ratifies the thermal table rather than changing it, and **the derivation lands BIT-IDENTICAL**. What is inconsistent is `hp`: it does two unrelated jobs, structural integrity *and* fuel store, and they want different numbers — a massive wood tile is 139 kg but `hp = 60` implies 32 kg of fuel (**4.4× off**), furniture 8.9× off. **Erik: *"i think u may change the hp to make it consistent."*** Fuel therefore separates from `hp` and derives from combustible mass; `hp` stays structural. **Split by feel**: the `thermal_mass` derivation is behaviour-neutral and lands at **T3b**; the fuel separation changes burn durations and lands at **T5**, where Erik plays. |
...
- `ACCEPTED GAP:` **Conduction is negligible at tile scale** (R10) — and T3 made it
  concrete: under real rates a solid–solid face is **identically zero below a dead
  band of `2^s/65536` K** — 4 K for steel, 64 K for glass, **256 K for wood**,
  bisected on the engine with the prediction hit to the digit. So solid–solid
  conduction is a threshold process, not a slow drift. Solid–GAS faces are
  unaffected: they run on ledger 7a's convection coefficient. Heat moves by
...
| **T3b** | **Materials authored by density** (R14). A `density` column per `[materials.*]` row; `thermal_mass` DERIVED at load from `ρc · V_tile / unit`, snapped to a power of two (R5), with `V_tile` from the level's `tile_size_m` and `ceiling_h`. Fuel is NOT touched here. The key design call: the material table is global while `tile_size_m` is per level, so the implementer must decide where the derivation lives | subagent, worktree | **Opus 5** | **BIT-IDENTICAL**: the derived `heat_inv_shift` must equal today's on every shipped level, so no golden moves and no digest bumps. T3 proved this is achievable — D1 was a no-op | — |
| **T4** | **Retire the dead test surface, before it can go red.** The 46 old-law tests (design v3 §11.1), `test_cool_shift_axis.py` (25), `test_temperature_cooling.py` (9), the CUDA check pair, the gate-A capture, `test_thermal_mass_axis.py`'s exact-value pin (rewritten to the R14 property: `thermal_mass` equals `ρc·V/unit` snapped, which is far stronger than either the old snapshot or a generic ordering assertion), and the R11 sustain check. Each deletion names the property that replaces it | subagent, worktree | **Haiku 4.5** | **suite green** — the old law is still live, so this only removes tests whose replacements already pass | — |
...
| **T6** | **Delete the old-law code**: `cast_fire_heat` + both call sites, the dead dials, `RAD_LIM_SHIFT`, the pair budget, the CUDA cast bindings, the five tool edits. **NOT the fire timer** (`wall_damage`): it stays live until the post-P7 fire arc (2026-09-23) | subagent, worktree | **Haiku** | suite-gated, mechanical | — |
| **T7** | **CLAUDE.md rules walkthrough** — every canonical row re-read against code after this arc removed systems | inline | **Opus** | every row points at code that exists | **review** |
| **T8** | **The ill-posed-test sweep** (R11's generalisation): find and remove tests that pose questions the model cannot answer cleanly. **Its own patch, after the rest.** **DEFERRED to an issue, after P7 (Erik, 2026-09-23)** | subagent, worktree | **Opus** | each removal names why the question was ill-posed | **review** |
