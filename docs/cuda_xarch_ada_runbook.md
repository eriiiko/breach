...
means → P3 statuses `6d690fda…`, 2026-07-05: the synced unit record grows the
status/condition list (the `__unit_status__` sub-hash); no field trajectory moved →
**P4 wave-push `07c3f370…`**, 2026-07-05: shockwaves displace units + trigger
KNOCKED_DOWN; the A/B pulse sub-tile-nudges the marine so only `__unit_pos__`
moved — no tile crossing, stamp + every field trajectory byte-identical).
On Ampere, the GPU path reproduces
...
and (b) the CPU path reproduces `07c3f370…`. **If all the gates go green on the Ada box,
...
2. Run the CUDA gates (the real attestation):
   ...
   reproduces the golden `07c3f370…`.
...
3. (Optional, holistic) A single combined digest with ALL backends on:
   - Note: `tests/xarch_digest.py` is an **S0 seed** — it captures the trajectory + digests it
     ...
     `.pyd`, and confirm the printed digest == `07c3f370…`; or (b) just rely on the per-solver
