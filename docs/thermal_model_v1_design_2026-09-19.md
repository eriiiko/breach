...
| lens | remit | verdict |
|---|---|---|
| **L1 — physics and thermodynamics** | Is the model sound? Are the three loss channels (§3.1) actually complete once `cool_shift` dies, or is there a fourth the design has not noticed? Is the currency argument (§3.2) right, and is the ~100× air figure correct? Does `ΔT = E/C` hold everywhere it is applied? Are the ledger's derivations (§4) the right ones? | **2 BLOCKING, 5 REQUIRED, 6 NOTE** — `thermal_model_v1_critique_L1_physics_2026-09-19.md`. **B1: §3.1's "exactly three routes" is false, and `foliage` ends up STRICTLY ADIABATIC.** `furniture`/`kindling`/`foliage` all have `conductivity = 0.0`, which is `NO_FACE` from both sides — route 3 is structurally absent, not slow. `foliage` also has `heat_atten = 0.0`, so `abs_mat = emitted = 0` and route 1 is gone too; `k_leak` debits the STREAM, never the cell's stored energy, so it cannot help. It is `flammable = true`, combustion writes into it (`combustion.cpp:1038` object_site), and its own config comment already says `cool_shift` is its only channel — so R1 turns it into an energy ratchet to `T_MAX_PHYS` that re-ignites neighbours forever, silently. **Orchestrator verified: exactly one row, and the row comment confirms it.** **B2: ledger item 6 is not derivable as §4 claims.** Conduction's ABSOLUTE rate comes from `SHIFT_AT_REF`/`KAPPA_REF`, a CFL stability anchor; literature κ sets only the RATIOS. Overspeed vs physical κ at 0.333 m / 24 Hz: **~48 500× (wood), ~51 200× (steel)** — orchestrator's independent check gives 43 124× for wood, same finding. Real wood takes **36 h** to conduct across one tile. "Derivation" and "test against the analytic slab" are mutually unsatisfiable without a ruling. **R2c**: R5's "6–10 %" quantisation silently assumes the **0.9** pin; at the shipped 0.7 it is −26 % / +40 %. And `rad_scale_derived` (item 2, marked Done) is DOWNSTREAM of the pin ruling — 2.1256e-08 at 0.7 vs 1.6533e-08 at 0.9. **R2b**: `A_rad` excludes floor/ceiling on an inverted argument (`k_leak` dilutes transport, it emits nothing), so every cell under-emits 6.2 %. **§5**: `k = 0.10` is RIGHT (max 8.2 % vs the exact slab law `S(r)=a/√(a²+r²)`) but R2's "~6 % of radiating area … same ballpark" is a FALSE corroboration — 0.0666 is 46.5 % off at r=12; two different derivations, one right, presented as agreement. **Verified sound and said so**: the currency really is one unit; the ~100× air figure is exact at **101.56×**; `ΔT = E/C` holds at every site including the varying-`N` gas branch (the `min(N,1)` absorption and the `N` divisor cancel exactly). |
| **L2 — determinism, integers, CUDA** | The two new planes against the number-ingress doors and `/fp:strict`; the digest and golden consequences; CPU↔GPU bit-identity for a per-cell leak and a per-cell ambient; the int64 headroom; the #54 closure identity with a channel deleted; whether T1's "uniform case reproduces the scalar era" is actually achievable bit for bit | **2 BLOCKING, 11 REQUIRED, 5 NOTE** — `thermal_model_v1_critique_L2_determinism_2026-09-19.md`. **B1: `t_amb_q` IS THE WRONG SCALAR — R3 as written delivers nothing.** In `radiation_sweep.cpp`, `t_amb_q` is the ABSOLUTE-ZERO OFFSET of the temperature scale (`T_abs = temperature[i] + t_amb`, feeding the Fleck denominator), bound from the same `293 << 16` arc #54's gas-energy seam runs on. The ambient the sweep actually RADIATES at is `amb_m = (e_table[0] * w_m) >> 16`, from `E°[0]` — it never reads `t_amb_q`. So promoting `t_amb_q` to a plane gives per-tile absolute-zero offsets (meaningless) and leaves `amb_m`, `ret`, `emit_body` and the Pass-1 clamp ceiling all pinned at 293 K. **Orchestrator verified at the source lines.** The critic also caught that §6 item 4 already names the right quantity ("breaks if `amb_m` is baked from a global") while the patch row and the draft rule name the wrong one. **FIX: the per-tile quantity is `amb_m`/`E°[0]`, not `t_amb_q`** — a design correction, no ruling needed. And all four of §6's T1 gates PASS against the wrong implementation, so the gate list could not have caught it. **B2: the "one golden re-baseline" is not achievable as cut.** `DIGEST_SPEC_VERSION` is salted into the hash before any field (`field_digest.py:121`), so T4a's spec-v6 membership bump moves EVERY golden in T4a's own commit, per CLAUDE.md's same-commit rule — while the re-baseline sits in T4b. **FIX: the digest bump moves to T4b**, which is what design v3 row 28 intended ("the same commit as P3's re-baseline"). **Also flagged: the `cool_shift` deletion surface is far larger than §7.3 admits** — a CUDA Pass 3, plus TWO entries in a PINNED POSITIONAL counter enum (`C_COOL = 3`, `C_THERMOSTAT = 12`, `C_SLOTS = 13`) folded BY INDEX at `physics_engine.cpp:377-388`, plus `e_cool_sum` with consumers in four tests and `storm_ledger.py`. Removing enum entries renumbers the rest silently. **Verified sound and said so**: the gather form's exact fixed point survives per-cell coefficients (`fb` as remainder is what makes it exact); conservation is structural and survives both planes; the int64 headroom argument is unchanged by a per-cell ambient (re-derived by induction against the `E°[3999]` ceiling); §6 item 8's six-group count is right and the thermostat is a TERM, not a group; the radiative fold needs no new counter because `e_solid_deposit_sum` books the post-clamp applied ΔT. |
| **L3 — scope, regression, systems reuse** | Does §7.1 miss an existing canonical system this design must use? Does §7.2 propose something that duplicates one? Does the patch cut (§5) leave a broken intermediate, or a state that cannot be gated? Are the test dispositions and the single re-baseline honest? Is anything in design v3 §11 silently contradicted? | **3 BLOCKING, 16 REQUIRED, 11 NOTE** — `thermal_model_v1_critique_L3_scope_2026-09-19.md`. **B1: every test deletion is scheduled AFTER the patch that invalidates it, and the biggest block is owned by no patch at all.** §7.3 lists four `cool_shift` items; the real surface is **43 non-doc files** (verified), including two DEDICATED test files — `test_cool_shift_axis.py` (25 tests) and `test_temperature_cooling.py` (9) — which die with R1 but are NOT among design v3 §11.1's 46 old-law tests that T5 inherits. So no §5 row owns them, while T4b's gate is "full suite", which would be red. Same defect twice more: `test_thermal_mass_axis.py:76` pins hull/steel 32, glass 16, wood 8 as "LIVE TUNED physics" and must fail when T4b applies R5's snapped rows (and is itself a SNAPSHOT test by CLAUDE.md's own standard); and `test_fire_heat_source.py` drives full `Simulation.step()` in nine tests pinning the old law but is deleted at T5, two patches after T4a breaks it. **B2: `materials.py:796-805` runs a LOAD-TIME fire-seed sustain check whose gain is literally `bed_per_I · 2^(cool_shift − heat_inv_shift)`** (verified). R1 deletes the channel it is derived from, nothing owns re-deriving it, and what it guards — can a fire seed sustain itself — is exactly T4b's HUMAN-TEST question. **B3: §7.1 names no SOURCE for the two per-tile planes.** "The same shape as `heat_atten_q`" means a material column, but a floor grate, a deck above and ship-vs-space are not material properties, and the material id budget is FULL (arc #60). The unnamed, unbudgeted systems are `level_lib`/`level_loader` and the editor. **Also**: `k_leak_q` ALREADY goes through `optics_fixed` (`physics_runner.py:465`), so it is an EXTENSION of the Extinction-planes system, not a new one; the `t_amb_q` draft rule ("never a global scalar") contradicts the gas-energy seam's born-at-ambient rule; the header claims to rule design v3 §12 item 5 while §4 says the 0.7-vs-0.9 pin is still open AND an unscheduled input to T3; **§5's own ordering constraints are violated by §5's own table** (`k_leak` is not live before the flip, and T4a IS a second moment where the game changes); and "A/B lockstep" cannot gate a deliberate behaviour change. **Verified sound and said so**: the shadow-first shape genuinely moves nothing live for T1–T3; §7.1's entries are correct as far as they go; §7.3's ledger-group bullet is right and non-obvious; R9's 459 %/2419 ≈ 0.2 % checks out; and **no parallel solver, plane store or transport path is proposed anywhere** — every duplication finding is at the rules layer, not in the code. |
