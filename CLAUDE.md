...
| Temperature solver | `cpp/src/temperature_solver.*` + `cuda_temperature.cu` | Heat deposits are Q16.16 saturating adds; **SOLIDS' temperature is derived here alone — GAS temperature is the energy field's mirror** (arc #54: the gas side's Pass-1 deposit and Pass-2 conduction sum go through the gas energy seam and the endpoint divide is deleted; `gas_energy == nullptr` keeps the whole pre-#54 T-form law bit-identical for direct-binding callers). **Ray-engine-v2 P1: the Pass-1 maximum-principle clamp `T ← min(T_after, max(T_before, E°⁻¹(Φ)))` lives HERE (between the saturating add and the rails, counted in `rad_clamp_hits`), dormant on the live path until P3 (`rad_fluence`/`e_table` nullptr); the CUDA twin gets it at P3** |
| Radiation sweep (shadow, P1) | `cpp/src/radiation_sweep.*` (step 2b of `PhysicsEngine::step_tail`) | THE radiative transport path (ray-engine-v2, design v3 §2.3–2.8): ONE traversal per ordinate in GATHER form, exact-integer conservative (`Σ rad_net + Σ rad_flux + Σ rad_amb ≡ 0` in int64); every payload is a channel on this sweep, never a second traversal; the transport step (`TRANSPORT_SHEAR` for heat, step for light at P6) is a parameter, never a fork; per-ordinate constants are checked-in literals (never `std::cos` at load); pure integer, pinned 0/0/0 on the ratchet. **The Fleck emission damping is ONE exact kit floor-division per cell per tick, on the EXCESS only: `f_q24 = floordiv_q(T_abs_q << 24, max(T_abs_q, 4·L_q))`, i.e. α = max(0, 1 − 1/g) — ALPHA FLOOR 0, Erik's ruling of 2026-09-16 (design row 39, §2.8's RULED paragraph), so `f == 2²⁴` exactly wherever the explicit material update is provably monotone (g ≤ 1: 1068 game for wood, 1424 for the a = 0.5 rows) and a combustion-held tile radiates full black body there. Never α = ½ (Fleck's own IMC bound, superseded at P2a), never a re-derived reciprocal.** Until P3 it writes the SHADOW planes `rad_net_sweep`/`rad_flux_sweep`/`rad_amb_sweep`/`rad_fluence` (int64) while the old cast still feeds the fold's int32 `rad_net`; P3 flips the fold, P4 adds the CUDA twin. **`run()` OVERWRITES those four — it zeroes them itself before its first ordinate and the conductor does NOT wipe them (P2a, design row 38), so they hold the last run's values at render time, which is what the tile inspector reads; a new output plane on this sweep joins that zeroing, never a caller's `fill(0)`.** |
| Emissive table | `cpp/src/emissive_table.{h,cpp}` (owner: `PhysicsEngine::emissive`) | THE `E°(T)` map, its inverse `e_inv_q` (fixed 12-trip, bucket LOW edge, 0 below `E°[0]`, saturates at 15996 game) and its bake; the bake BODY is out of line in the `.cpp` on `/fp:strict` — never a bake in a header (a header-inline bake compiles under `bindings.cpp`'s fast-math and can FMA-contract `k4·scale + 0.5`); one implementation, two owners until P3 (`Raycaster::bake_emissive_table` calls it); `tests/test_emissive_table.py` asserts the two tables identical and equal to the reference bake |
| Extinction planes | `GameMap.heat_atten_q` (static, from `MaterialTable.heat_atten_q16`) / `dyn_heat_atten_q` (the fourth `stamp_units` output) / `src/simulation/optics_fixed.py` | Every optical interaction the sweep's channels see is a Q16 coefficient on these planes; `0 ≤ a ≤ d ≤ ONE` is an ingress invariant at the materials door (`heat_atten ∈ [0,1]`, `heat_atten > 0 ⇒ thermal_mass > 0`, `tests/test_optics_ingress.py`) re-checked by the sweep; dynamic stamps are MAX, never sums; the material share `a` emits and absorbs, the body share `d − a` absorbs and re-emits AMBIENT (Erik's ruling, design row 25). `optics_fixed` RAISES outside [0, 1] — never clamps. `dyn_heat_atten_q` enters the digest at P3, not before |
| Integer reference | `docs/ray_engine_v2_scheme_study_2026-09-13/sweep_ref_q.py` + `sweep_ref_q_gates.py` (guarded by `tests/test_ray_engine_v2_integer_reference.py`) | THE executable specification of the sweep's integer arithmetic (the Fleck pre-pass, the E-table and its inverse, the sweep, the radiative fold's clamp). Any change is made THERE FIRST, its gates re-run, and the C++ written against it — never the other way round; `tests/test_radiation_sweep_reference.py` (gate 0) holds the C++ bit for bit to it. Not engine code: nothing in `src/` or `cpp/` imports it |
