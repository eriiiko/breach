...
> **Last updated**: 2026-09-23 — every row re-verified against the code after
> the thermal / ray-engine arc (#12, T7).
...
- **Python: always this machine's breach env** — it differs per machine and
  `docs/dev_setup.md` names each one; bare `python` may be a different install
  and fails breach imports with a misleading ModuleNotFoundError.
- **pytest: always `pytest tests -q`** — never bare `pytest` from repo root.
- C++/CUDA: `cpp/` via CMake; per-machine build scripts `cpp/build_*.bat`
  (which one where: `docs/dev_setup.md`). CPU build → `cpp/build/Release`;
  CUDA build → `cpp/build_cuda/`.
...
| Starting a fire | `payloads.py::ignite_ring` (gameplay) · `combat.py::apply_temperature_ignition` (the emergent path) · `debug_keys.py::debug_ignite` (dev key) | **A fire is started by DELIVERING HEAT, never by writing `fire` alone.** Since R3 the O2 demand and fuel destruction ride `hotf = clamp((T−fire_T_ext)/fire_T_span, 0, hotf_cap)`, so a flame seeded on an ambient tile draws no O2, burns no fuel and — `k_die`'s e-fold being 125 s — does not even go out: a decorative fire that consumes and spreads nothing. Any new igniter (payload row, weapon, editor tool, test fixture, bench) seeds the tile AT OR ABOVE its own `ignition_temp` as well as lighting it, or it does nothing at all. `ignite_ring` now carries `IGNITE_HEAT` for exactly this; prefer heat-only (the flamethrower's `heat_deposit`) and let `apply_temperature_ignition` do the lighting where the one-tick delay is acceptable. Incidents, all 2026-09: both incendiary ammo rows inert (#65), the dev key dead on arrival, and five test fixtures silently measuring nothing |
| O2 pressure factor | `cpp/src/o2_pressure_factor.h` (edges `[physics.fire] p_ext_atm` / `p_full_atm`) | THE pressure gate on the O2 sustain law (#7): one FP_HD `factor` read by BOTH law sites (the fire ODE's `o2f`, combustion's claim gate `o2f_j`) on both backends, multiplying the mole-fraction law — 0 below `p_ext`, exactly 1 at or above `p_full`. Its input is the materialized `gmap.atmosphere` over the same air the mole fraction reads — never density (hot air is thin, not low-pressure: the July density-trap fix), never a re-derived p*. A new O2 read goes through it |
...
| Material / gas tables | `src/simulation/materials.py` / `gases.py` | New material/gas = a table row, never a hardcoded id or per-material if. **ID BUDGET FULL (arc #60): id 9 (foliage) numerically == the CSV SPACE_CODE — harmless, loader-intercepted, but an 11th material needs Erik's SPACE_CODE-migration ruling FIRST**. **Row VALUES are placeholders (Erik, 2026-09-22); the thin-rows material rulings ("no massive object ever burns"; the 6 mm lumped door, `THIN_LIMIT_M`/`LUMPED_CRITERION_EXEMPT`) are PROVISIONAL until his material design session — `docs/thin_material_rows_design_2026-09-20.md` header** |
...
| Energy closure identity | `EOSSolver` / `TemperatureSolver` / `CombustionSolver` `e_*_sum` counters + `GameMap.gas_energy_seam_net()` | Every channel that moves `gas_energy` books itself, and since P-G1b the identity holds ACROSS WHOLE TICKS in int64: `Δ Σ_accountable gas_energy == EOS + thermal-solver-gas-side + combustion + seams`. A new writer adds a counter AND a term in ONE of those four groups — there is no fifth, bar the one existing boundary export every gate already carries, `−PhysicsEngine.e_water_evac_export_sum` (water evaporation, arc #54 §2.7). Radiation into gas (P5c) is the thermal-solver group's `e_gas_deposit_sum`; `e_rad_clamp_drop_sum` is energy that never ENTERED the books (the clamp's withheld gain), so it is in no group — and so are its two §8.4 siblings (P5c follow-up): `e_rad_boundary_export_sum` (rad_net on a gas cell OUTSIDE the accountable set — ring, breach — exported, never landed) and `e_rad_floor_drop_sum` (what the floored chain does not land below `n_floor_heat`). A radiative energy that does not land is COUNTED on the boundary, never made a term here: it never touched `gas_energy`, so a term would break the identity by its own amount. Gated by `test_e1_hot_rail.py::test_no_transport_mint`, `test_gas_energy_tile_flip.py`, and the §6 benches (`_sealedbox_bisect`, `_fire`, `_vent`, `_blast`, `_quiet_books`). The ledger spans gas AND solids (P-G5): `TemperatureSolver.solid_energy_books_sum` (Σ thermal_mass_raw·T_raw, a snapshot) closes against `e_solid_deposit_sum` + `e_solid_cond_sum` + `CombustionSolver.e_comb_solid_heat_sum`. **Nothing relaxes to ambient**: the thermostat (Pass 3, `cool_shift`) is DELETED and its term REMOVED, not zeroed (T5b, thermal v2 R1; `docs/ray_engine_v2_scheme_study_2026-09-13/report_t5b.md` §7.3) — gated by `test_thermostat_books.py` |
| Temperature solver | `cpp/src/temperature_solver.*` + `cuda_temperature.cu` | Heat deposits are Q16.16 saturating adds; **SOLIDS' temperature is derived here alone (bar combustion's booked object-site deposit) — GAS temperature is the energy field's mirror** (arc #54: the gas side's Pass-1 deposit and Pass-2 conduction sum go through the gas energy seam and the endpoint divide is deleted; `gas_energy == nullptr` keeps the whole pre-#54 T-form law bit-identical for direct-binding callers). **The Pass-1 maximum-principle clamp `T ← min(T_after, max(T_before, e_ceiling_q(Φ)))` lives HERE (between the saturating add and the rails, counted in `rad_clamp_hits`), LIVE on CPU and CUDA since T5b, fed the sweep's `rad_fluence` and `engine.emissive`'s table; `nullptr` for either means no clamp (the direct-binding path). Its ceiling is `e_ceiling_q` at all four sites (solid + gas branch, both backends) since P5d — one bucket of headroom, Erik's ruling of 2026-09-24 — never `e_inv_q`**. **The radiation fold has a GAS branch since P5c (design §6.3, `sweep_ref_q.fold_pass1_gas`, gate G15): an ACCOUNTABLE gas cell's `rad_net` goes through the staged `deposit_dT_wide_i64` chain in the fold's own gas currency, magnitude then sign, and lands as `dE = N·(T_target − T_before)` with `T_before = mirror_q(E, N)` through `deposit_railed` — never a `temperature[i] =`, never a scaled dE, never the design's letter `N·(T_target + t_amb) − E` (it drains the residual `E mod N`) — booked in `e_gas_deposit_sum` (group 1, no new group); no `min(N, N_AMB)/N_AMB` factor (the density law is in `a_gas`); a cell with `rad_net == 0` is not touched. The clamp's WITHHELD energy, on gas and solids, is `e_rad_clamp_drop_sum` (priced at `cap_real_`; CUDA slot 12, APPENDED) — a §8.4 boundary counter, NOT a closure term. NOTHING LEAVES THAT BOUNDARY UNCOUNTED (P5c follow-up): a gas cell outside the accountable set books its whole rad_net in `e_rad_boundary_export_sum` (slot 13), an accountable cell below `n_floor_heat` the floored chain's unlanded remainder in `e_rad_floor_drop_sum` (slot 14), both signed, in the same heat currency, counters only — so `Σ rad_net − Σ landed − those three` is the conversions' declared rounding, gated per tick on live breached rooms by `test_temperature_gas_radiation.py::test_closure_identity...`.** **`n_floor_heat ≥ 0.01` is LOAD-BEARING for gas radiative stability** (the reference's gate 14: at ≤ 0.005 even the Fleck-damped gas step undershoots below ambient) — never lower it without re-running G14 and G15 |
| Radiation sweep | `cpp/src/radiation_sweep.*` (step 2b of `PhysicsEngine::step_tail`) + its CUDA twin `cpp/src/cuda_radiation_sweep.*` (`set_radiation_backend`) | THE radiative transport path (ray-engine-v2, design v3 §2.3–2.8), LIVE since T5b: the temperature fold reads `rad_net_sweep`, the Pass-1 clamp `rad_fluence`, units `rad_flux_sweep` (`exchange.py`). The old `gmap.rad_net`/`rad_flux`/`rad_amb` planes carry nothing since T6 deleted their cast (`rad_net` survives only as a dead `step_tail` argument, recorded debt) — never read them. ONE traversal per ordinate in GATHER form, exact-integer conservative (`Σ rad_net_sweep + Σ rad_flux_sweep + Σ rad_amb_sweep ≡ 0` in int64); every payload is a channel on this sweep, never a second traversal; the transport step (`TRANSPORT_SHEAR` for heat, step for light at P6) is a parameter, never a fork; per-ordinate constants are checked-in literals (never `std::cos` at load); pure integer, pinned 0/0/0 on the ratchet. **The heat channel's currency is 2^k finer than a heat count (`E_FINE_BITS`, `emissive_table.h`; #78): Φ, the table, the ambient levels and the four planes are all in it, and every reader of a sweep plane or an E° value converts ONCE through `fixedpoint::fine_heat_shr` (the gas chain at `deposit_dT_wide_i64`'s final narrow, its `fine_bits`), never an inline shift — Python twins `sweep_ref_q.fine_heat_shr` / `optics_fixed.fine_heat_shr`, display and reports through `optics_fixed.dequantize_heat`, the planes' currency from `optics_fixed.sweep_fine_bits`.** **The CUDA twin (P4) is the same formulation integer for integer**: an arithmetic change lands in the reference, then `radiation_sweep.cpp`, then `cuda_radiation_sweep.cu`, and `tests/cuda_radiation_sweep_check.py` holds GPU == CPU at tol 0 on every plane and counter; its books use the unsigned-long-long `atomicAdd` idiom, never float; its `(N, h, w)` launch core counts and masks an illegal env instead of throwing; ordinate constants come from `RadiationSweep::ordinate_table`. **The Fleck emission damping is ONE exact kit floor-division per cell per tick, on the EXCESS only: `f_q24 = floordiv_q(T_abs_q << 24, max(T_abs_q, 4·L_q))`, i.e. α = max(0, 1 − 1/g) — ALPHA FLOOR 0, Erik's ruling of 2026-09-16 (design row 39). Never α = ½, never a re-derived reciprocal.** **L has TWO arms (P5b): a thermal solid's material excess through its `heat_inv_shift`, and an absorbing gas cell's smoke-term excess `a_gas·(E°[T] − E°[0])` through `fleck_L_gas_q` in the temperature fold's OWN gas currency (N floored at `n_floor_q`, `reciprocal_q16`, `recip_cv`, the staged `deposit_dT_wide_i64`) — which `PhysicsEngine::gas_capacity_q()` derives from `temperature.c_v` / `.n_floor_heat` and step_tail hands both backends: never a second gas currency, and the arm keys on `a_gas`, so it engages exactly on gas cells whose smoke term absorbs (since P5c the shipped smoke does) and every other gas cell stays at `f == 2²⁴`** (`tests/test_radiation_sweep_gas_fleck.py`; the property it holds — monotone gas cooling, never below ambient — is the reference's gate 14). At the shipped derived scale `f == 2²⁴` across the fire range (pinned by `test_m3_derived_h_bed.py::test_the_fleck_damping_stays_inert_through_a_full_corrected_burn`). **`run()` OVERWRITES its four int64 planes — it zeroes them itself before its first ordinate and the conductor does NOT wipe them (design row 38), so they hold the last run's values at render time, which is what the tile inspector reads; a new output plane on this sweep joins that zeroing, never a caller's `fill(0)`.** |
| Emissive table | `cpp/src/emissive_table.{h,cpp}` (owner: `PhysicsEngine::emissive`) | THE `E°(T)` map, its bake, its inverse `e_inv_q` (fixed 12-trip, bucket LOW edge, 0 below `E°[0]`, saturates at 15996 game — the radiation temperature), and the clamp's ceiling `e_ceiling_q` (P5d: the top of the first bucket out-emitting Φ; 0 below `E°[0]`; saturating below `T_MAX_PHYS`) — the clamp reads the ceiling, never `e_inv_q`; **row 31's 0 ceiling below `E°[0]` stays** — #78 measured it dormant: in the fine currency near-ambient cells still take that branch (the ambient field's Φ sits up to 15 fine counts below `E°[0]`) but book at most 0.0063 heat counts a tick there, under one temperature LSB for every shipped row, so the clamp binds on it nowhere on the P5d bench (0 hits, against P5d's ~1 kW); **a table carries its CURRENCY, `fine_bits` (default `E_FINE_BITS`, #78)**: the same bake at `rad_scale · 2^k` (one rounding), refused outside `[0, E_FINE_BITS]` and at a top entry ≥ `E_TABLE_TOP_MAX` (2^44, the sweep's int64 headroom) — so a RESOLVING-scale table is baked coarse (`fine_bits = 0`), and a refused bake leaves the cache as it was; the bake BODY is out of line in the `.cpp` on `/fp:strict` — never a bake in a header (a header-inline bake compiles under `bindings.cpp`'s fast-math and can FMA-contract `k4·scale + 0.5`); the live table is baked at `[physics.radiation] rad_scale_derived`, the ONLY emission-calibration key since T6. One implementation, two owners: `engine.emissive` (live) and the vestigial `Raycaster` copy (never marched against since T6; `Raycaster::bake_emissive_table` calls the same bake) — `tests/test_emissive_table.py` asserts the two tables identical at equal dials and equal currency, and equal to the reference bake |
| Extinction planes | `GameMap.heat_atten_q` (static, from `MaterialTable.heat_atten_q16`) / `dyn_heat_atten_q` (the fourth `stamp_units` output) / `src/simulation/optics_fixed.py` | Every optical interaction the sweep's channels see is a Q16 coefficient on these planes; `0 ≤ a ≤ d ≤ ONE` is an ingress invariant at the materials door (`heat_atten ∈ [0,1]`, `heat_atten > 0 ⇒ thermal_mass > 0`, `tests/test_optics_ingress.py`) re-checked by the sweep; dynamic stamps are MAX, never sums; the material share `a` emits and absorbs, the body share `d − a` absorbs and re-emits AMBIENT (Erik's ruling, design row 25). `optics_fixed` RAISES outside [0, 1] — never clamps. `dyn_heat_atten_q` is digested (spec v6, T5b); its static twin `heat_atten_q` deliberately is not (a pure projection of `material`). **The GAS term (P5a, design §6.3) is folded in by the sweep's own pre-pass, never stored as a plane:** on a gas cell `a = max(a, a_gas)`, `d = max(d, a)` with `a_gas = min(ONE, Σ_g heat_absorb_q16[g]·max(0, N_g) >> 16)`, 0 below `gas_energy::N_EPS_RAW` bulk, read from `GameMap.gas`, `GasTable.heat_absorb_q16` (door `[0, 4096]` in `gases.py`: the int64 sum's headroom, not physics) and `step_tail`'s bulk sum; a thermal solid ignores the gas in its pores. LIVE since P5c: exactly ONE gas absorbs — `[gases.smoke] heat_absorb`, DERIVED in config.toml (soot's IR mass-specific extinction, Widmann et al. 2003, through the engine's own soot bookkeeping and tile geometry) and re-derived by `tests/test_gas_heat_absorb.py` from the live dials, so moving `soot_yield`, `KG_FUEL_PER_N_O2` or the tile geometry means re-deriving it; every other gas is 0.0 with its reason (physical, or an ACCEPTED GAP: band absorbers are not modelled; soot is grey) |
| Integer reference | `docs/ray_engine_v2_scheme_study_2026-09-13/sweep_ref_q.py` + `sweep_ref_q_gates.py` (guarded by `tests/test_ray_engine_v2_integer_reference.py`) | THE executable specification of the sweep's integer arithmetic (the Fleck pre-pass, the E-table and its inverse, the sweep, the radiative fold's clamp, and since P5c the fold's gas branch, `fold_pass1_gas`). Any change is made THERE FIRST, its gates re-run, and the C++ written against it — never the other way round; `tests/test_radiation_sweep_reference.py` (gate 0) holds the C++ sweep bit for bit to it, and `tests/test_temperature_gas_radiation.py` the C++ fold. Its DEFAULT E-table is a RESOLVING scale for the arithmetic gates, read from no config, in whole heat counts; a number about the SHIPPED game is measured on `E_LIVE`, which is in the fine currency (`FINE_BITS` == the engine's `E_FINE_BITS`; a table carries its own `fine_bits`, #78), and `config_dials_match()` guards `RAD_SCALE_LIVE` against `[physics.radiation] rad_scale_derived` (M3). Not engine code: nothing in `src/` or `cpp/` imports it |
...
| Tile inspector | `renderer/hover_readout.py::pack_hover_readout` | THE per-tile debug readout (F6 hides) — tools/HUD read it, never roll a parallel field probe; future resident gather swaps inside this seam. Its sweep rows show what the sweep READS (P5c): on a gas cell the effective `a`/`d` with the smoke term and the Fleck GAS arm's `f`, formed by the engine's own FP_HD one-cell functions (`RadiationSweep.gas_extinction_q16` / `fleck_f_gas_q24`), never a copy of the law |
