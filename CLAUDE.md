...
| Radiation sweep (shadow, P1) | `cpp/src/radiation_sweep.*` (step 2b of `PhysicsEngine::step_tail`) | THE radiative transport path (ray-engine-v2, design v3 §2.3–2.8): ONE traversal per ordinate in GATHER form, exact-integer conservative (`Σ rad_net + Σ rad_flux + Σ rad_amb ≡ 0` in int64); every payload is a channel on this sweep, never a second traversal; the transport step (`TRANSPORT_SHEAR` for heat, step for light at P6) is a parameter, never a fork; per-ordinate constants are checked-in literals (never `std::cos` at load); pure integer, pinned 0/0/0 on the ratchet. **The Fleck emission damping is ONE exact kit floor-division per cell per tick, on the EXCESS only: `f_q24 = floordiv_q(T_abs_q << 24, max(T_abs_q, 4·L_q))`, i.e. α = max(0, 1 − 1/g) — ALPHA FLOOR 0, Erik's ruling of 2026-09-16 (design row 39, §2.8's RULED paragraph), so `f == 2²⁴` exactly wherever the explicit material update is provably monotone (g ≤ 1: 1068 game for wood, 1424 for the a = 0.5 rows) and a combustion-held tile radiates full black body there. Never α = ½ (Fleck's own IMC bound, superseded at P2a), never a re-derived reciprocal.** Until P3 it writes the SHADOW planes `rad_net_sweep`/`rad_flux_sweep`/`rad_amb_sweep`/`rad_fluence` (int64, per-tick, wiped by the conductor) while the old cast still feeds the fold's int32 `rad_net`; P3 flips the fold, P4 adds the CUDA twin |
