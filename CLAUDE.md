...
| Temperature solver | `cpp/src/temperature_solver.*` + `cuda_temperature.cu` | Heat deposits are Q16.16 saturating adds; **SOLIDS' temperature is derived here alone (bar combustion's booked object-site deposit) — GAS temperature is the energy field's mirror** (arc #54: the gas side's Pass-1 deposit and Pass-2 conduction sum go through the gas energy seam and the endpoint divide is deleted; `gas_energy == nullptr` keeps the whole pre-#54 T-form law bit-identical for direct-binding callers). **The Pass-1 maximum-principle clamp `T ← min(T_after, max(T_before, E°⁻¹(Φ)))` lives HERE (between the saturating add and the rails, counted in `rad_clamp_hits`), LIVE on CPU and CUDA since T5b, fed the sweep's `rad_fluence` and `engine.emissive`'s table; `nullptr` for either means no clamp (the direct-binding path)**. **The radiation fold has a GAS branch since P5c (design §6.3, `sweep_ref_q.fold_pass1_gas`, gate G15): an ACCOUNTABLE gas cell's `rad_net` goes through the staged `deposit_dT_wide_i64` chain in the fold's own gas currency, magnitude then sign, and lands as `dE = N·(T_target − T_before)` with `T_before = mirror_q(E, N)` through `deposit_railed` — never a `temperature[i] =`, never a scaled dE, never the design's letter `N·(T_target + t_amb) − E` (it drains the residual `E mod N`) — booked in `e_gas_deposit_sum` (group 1, no new group); no `min(N, N_AMB)/N_AMB` factor (the density law is in `a_gas`); a cell with `rad_net == 0` is not touched. The clamp's WITHHELD energy, on gas and solids, is `e_rad_clamp_drop_sum` (priced at `cap_real_`; CUDA slot 12, APPENDED) — a §8.4 boundary counter, NOT a closure term. NOTHING LEAVES THAT BOUNDARY UNCOUNTED (P5c follow-up): a gas cell outside the accountable set books its whole rad_net in `e_rad_boundary_export_sum` (slot 13), an accountable cell below `n_floor_heat` the floored chain's unlanded remainder in `e_rad_floor_drop_sum` (slot 14), both signed, in the same heat currency, counters only — so `Σ rad_net − Σ landed − those three` is the conversions' declared rounding, gated per tick on live breached rooms by `test_temperature_gas_radiation.py::test_closure_identity...`.** **`n_floor_heat ≥ 0.01` is LOAD-BEARING for gas radiative stability** (the reference's gate 14: at ≤ 0.005 even the Fleck-damped gas step undershoots below ambient) — never lower it without re-running G14 and G15 |
...
| Emissive table | `cpp/src/emissive_table.{h,cpp}` (owner: `PhysicsEngine::emissive`) | THE `E°(T)` map, its inverse `e_inv_q` (fixed 12-trip, bucket LOW edge, 0 below `E°[0]`, saturates at 15996 game) and its bake; the bake BODY is out of line in the `.cpp` on `/fp:strict` — never a bake in a header (a header-inline bake compiles under `bindings.cpp`'s fast-math and can FMA-contract `k4·scale + 0.5`); the live table is baked at `[physics.radiation] rad_scale_derived`, the ONLY emission-calibration key since T6. One implementation, two owners: `engine.emissive` (live) and the vestigial `Raycaster` copy (never marched against since T6; `Raycaster::bake_emissive_table` calls the same bake) — `tests/test_emissive_table.py` asserts the two tables identical at equal dials and equal to the reference bake |
