...
| Gas energy field | `gamemap.py::gas_energy` (+ `refresh_gas_energy`, `_gas_energy_accountable`) / `EOSSolver::step`'s `gas_energy` arg | THE conserved truth for gas thermal energy (int64, the exact unshifted `N_raw·T_abs_raw`, on the one canonical accountable set); `temperature` is its MIRROR on gas cells and stays the truth on thermal solids. Nothing writes gas `temperature` directly — tests and tools included: use `GameMap.seed_gas_temperature` |
...
| Energy closure identity | `EOSSolver`'s `e_*_sum` counters (eos_solver.h) | Every channel that moves `gas_energy` books itself; the seven-term identity in eos_solver.h holds exactly in int64 every EOS step. A new channel adds a counter AND a term — gated by `test_e1_hot_rail.py::test_no_transport_mint` and all five §6 benches |
