...
| Integer reference | `docs/ray_engine_v2_scheme_study_2026-09-13/sweep_ref_q.py` + `sweep_ref_q_gates.py` (guarded by `tests/test_ray_engine_v2_integer_reference.py`) | THE executable specification of the sweep's integer arithmetic (the Fleck pre-pass, the E-table and its inverse, the sweep, the radiative fold's clamp, and since P5c the fold's gas branch, `fold_pass1_gas`). Any change is made THERE FIRST, its gates re-run, and the C++ written against it — never the other way round; `tests/test_radiation_sweep_reference.py` (gate 0) holds the C++ sweep bit for bit to it, and `tests/test_temperature_gas_radiation.py` the C++ fold. Its DEFAULT E-table is a RESOLVING scale for the arithmetic gates, read from no config; a number about the SHIPPED game is measured on `E_LIVE`, and `config_dials_match()` guards `RAD_SCALE_LIVE` against `[physics.radiation] rad_scale_derived` (M3). Not engine code: nothing in `src/` or `cpp/` imports it |
